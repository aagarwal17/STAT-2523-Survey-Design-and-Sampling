...
library(readxl)
bat_dat <- read_excel("STAT 2523 DoE & Q Fall 2022/Lectures/Battery Design Example_V2.xlsx",
                      sheet = "Long Format")
head(bat_dat)
...
# By material
HSD.test(fit_modelLS, c("material"), console=TRUE)

# By temp
HSD.test(fit_modelLS, "temp", console=TRUE)

# By combination of material:temp
HSD.test(fit_modelLS, c("material", "temp"), console=TRUE)
